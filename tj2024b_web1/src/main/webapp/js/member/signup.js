...
const onSignUp = ( ) => { console.log('함수 실행됨.');
    ...
    let mphoneInput = document.querySelector('.mphoneInput'); console.log( mphoneInput )
    ...
    let mphone = mphoneInput.value;         console.log( mphone )
    // (!!!생략) 다양한 유효성검사 코드 생략 .
    // (3) 입력받은 값들을 서버에게 보낼 객체 만들기
    let dataObj = { mid : mid , mpwd : mpwd , mname : mname , mphone : mphone }; console.log( dataObj );
    // (4) fetch 옵션 
    const option = {   
